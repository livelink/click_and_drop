...
module ClickAndDrop
  class CreateOrderResponse
    ...
    # label in format base64 string
    attr_accessor :label

    attr_accessor :label_errors
    ...
    def self.openapi_types
      {
        ...
        :'label' => :'String',
        :'label_errors' => :'Array<CreateOrderLabelErrorResponse>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ClickAndDrop::CreateOrderResponse` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ClickAndDrop::CreateOrderResponse`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'label_errors')
        if (value = attributes[:'label_errors']).is_a?(Array)
          self.label_errors = value
        end
      end
    end
    ...
    def list_invalid_properties
      ...
      if @order_identifier.nil?
        invalid_properties.push('invalid value for "order_identifier", order_identifier cannot be nil.')
      end
    ...
    def valid?
      return false if @order_identifier.nil?
    ...
    def hash
      [order_identifier, order_reference, created_on, order_date, printed_on, manifested_on, shipped_on, tracking_number, label, label_errors].hash
    end
