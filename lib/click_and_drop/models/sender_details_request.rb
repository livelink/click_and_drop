  ...
  class SenderDetailsRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ClickAndDrop::SenderDetailsRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ClickAndDrop::SenderDetailsRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@trading_name.nil? && @trading_name.to_s.length > 250
        invalid_properties.push('invalid value for "trading_name", the character length must be smaller than or equal to 250.')
      end
    ...
    def valid?
      return false if !@trading_name.nil? && @trading_name.to_s.length > 250
    ...
    # @param [Object] trading_name Value to be assigned
    def trading_name=(trading_name)
      if !trading_name.nil? && trading_name.to_s.length > 250
        fail ArgumentError, 'invalid value for "trading_name", the character length must be smaller than or equal to 250.'
      end
    ...
    def hash
      [trading_name, phone_number, email_address].hash
    end
